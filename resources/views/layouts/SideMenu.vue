<template>
	<aside class="menu app-sidebar animated"
		...
		<div class="app-sidebar-inner">
			...
			<!-- LISTA DE MENUS -->
			<div v-for="(category, index1) in menuitems" v-bind:key="index1" class="is-marginless is-paddingless">
				<div class="menu-label" v-html="category.title" v-if="checkItemMeta(category.meta)"></div>
					...
					<li v-for="(item, index2) in category.items" v-bind:key="index2">
						<div v-if="checkItemMeta(item.meta)">
							...
							<router-link v-if="item.path" 
								...
								v-html="item.title" @click="closeSubmenu"></router-link>
							<!-- MENU COM CHILDREN -->
							<a v-else @click="toggleSubmenu($event)"
							...
							<expanding v-if="item.children">
								<ul v-show="submenuActive == `submenu-${index1}-${index2}`">
									<li v-for="(child, index3) in item.children" v-bind:key="index3">
										<router-link :to="child.name ? { name: child.name, params: child.params } : child.path" 
