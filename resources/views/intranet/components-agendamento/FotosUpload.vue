<template>
	<form name="formAgendamentoFotoUpload" enctype="multipart/form-data" novalidate>
	...
	methods: {		
		...
		save(formData) {
			...
			.then(response => {
				if (response) {
					vm.$toast.open({
						message: response.message ? response.message : 'Foto(s) salva(s) com sucesso.',
						type: 'is-success',
						position: 'is-top',
						duration: 5000
					});
		...
		upload(formData) {
			return new Promise((resolve, reject) => {
				this.axios.post('/agendamento/enviarFoto', formData, {
