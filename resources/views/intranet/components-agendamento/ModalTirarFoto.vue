		...
		<div class="card">
			<div class="card-image">
				<div v-show="showVideo">
					...
					<div class="is-fullwidth has-text-centered is-marginless is-paddingless">
						<button type="button" class="button is-white is-large" id="btn-snap"
							@click="snap">
				...
				<div v-show="showCanvas">
					<canvas ref="canvas" width="640" height="480" class="is-4by3" style="width:100%"></canvas>
					<b-field position="is-centered" grouped v-show="!isLoading">
						<p class="control">
							<button type="button" class="btn-canvas button is-danger is-medium"
								@click="descartar">
						...
						<p class="control">
							<button type="button" class="btn-canvas button is-success is-medium"
								@click="enviar">
...
export default {
	...
	methods: {
		...
		sendImage() {
			...
			.then(response => {
				if (response) {
					vm.$toast.open({
						message: response.message ? response.message : 'Foto salva com sucesso.',
						type: 'is-success',
						position: 'is-top',
						duration: 5000
					});
		...
		upload(image) {
			return new Promise((resolve, reject) => {
				this.axios.post('/agendamento/enviarFoto', {
	...
	mounted() {
		...
		if (navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
			...
			navigator.mediaDevices.getUserMedia({ video: { width: 800, height: 600, facingMode: "environment" } })
			...
			navigator.getUserMedia({ video: { width: 800, height: 600, facingMode: "environment" } }, 
			function(stream) {
				vm.mediaStream = stream;
				video.src = stream;
				video.play();
			...
			navigator.webkitGetUserMedia({ video: { width: 800, height: 600, facingMode: "environment" } }, 
			function(stream){
				vm.mediaStream = stream;
			...
			navigator.mozGetUserMedia({ video: { width: 800, height: 600, facingMode: "environment" } }, 
			function(stream){
				vm.mediaStream = stream;
				video.src = window.URL.createObjectURL(stream);
				video.play();
