<template>
	<div>
		<h1 class="title is-clearfix">Lista de matriz/filiais</h1>
		<div class="is-clearfix">
			...
			<b-table
				...
				<template slot-scope="props">
					<b-table-column field="mf.nomeFantasia" label="Nome fantasia" sortable>
						{{ props.row.nomeFantasia }}
					</b-table-column>
					<b-table-column field="mf.cnpj" label="CNPJ" sortable>
						{{ props.row.cnpj | formatCpfCnpj }}
					</b-table-column>
					<b-table-column field="e.cidade" label="Localização" sortable>
						{{ props.row.localizacao }}
					</b-table-column>
					<b-table-column field="mf.situacao" label="Situação" sortable>
						{{ props.row.situacao | capitalize }}
					</b-table-column>
					<b-table-column label="Ações" numeric>
						<router-link :to="{ name: 'cadastro-matriz-filial', params: {id: props.row.id} }"
...
export default {
	...
	methods: {
		...
		getList() {
			this.loading = true;
			this.axios.get(`/matrizfilial/getList/page=${this.page}&perPage=${this.perPage}&text=${this.text}`
