<template>
	<div class="columns is-centered">
		<div class="column">
			<h1 class="title is-clearfix"><span v-html="'Cadastro de servi&ccedil;o'"></span></h1>
			<b-message :closable="false">
				<form name="cadastroEmpresaServicoForm" method="post" action="#"
					...
					<div class="columns">
						<div class="column is-half-desktop">
							<select-ajax name="tipoServico" v-model="form.tipoServico.id" 
								size="is-medium"
						...
						<div class="column is-half-desktop">
							<select-ajax name="situacao" v-model="form.situacao" 
								size="is-medium"
								v-if="form.id > 0"
					...
					<div class="columns">
						<div class="column is-half-desktop">
							<date-time-picker placeholder="Tempo previsto"
								size="is-medium"
								name="tempoPrevisto"
...
export default {
	...
	methods: {
		...
		onSubmit() {
            this.form.post('/empresaServico/salvar')
			.then(response => {
				if (response.message) {
					this.$toast.open({
	...
	beforeRouteEnter (to, from, next) {
		...
		if (id) {
			Vue.axios.get('/empresaServico/findId/' + id)
			.then(response => {
				next(vm => {
	...
	beforeRouteUpdate (to, from, next) {
		...
		if (id) {
			this.axios.get('/empresaServico/findId/' + id)
			.then(response => {
