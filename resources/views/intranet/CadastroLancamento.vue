	...
	<div class="columns is-centered">
		<div class="column">
			...
			<b-message :closable="false">
				<form name="cadastroEmpresaServicoForm" method="post" action="#"
					...
					<div v-show="activeTab == 1">
						<div class="columns">
							<div class="column is-half-desktop">
								<b-field label="Tipo de lançamento" expanded>
									<b-field position="is-left" type="is-danger"
										:message="form.errors.get('tipo')" group-multiline>
										<b-field expanded>
											<b-radio-button v-model="form.tipo"
							...
							<div class="column is-half-desktop">
								<input-autocomplete name="tecnico"
						...
						<div class="columns">
							<div class="column is-3-desktop">
								<input-text name="valor" v-model="form.valor" 
									...
									size="is-medium"
									:placeholder="`Valor ${form.tipo == 'receita' ? 'recebido' 
									: form.tipo == 'pagamento' ? 'pago' : '' }`"
									:is-money="true"
									:has-error="form.errors.has('valor')"
									:error="form.errors.get('valor')"></input-text>
							</div>
							<div class="column is-3-desktop">
								<input-text name="valorCalculado" :value="valorCalculado()"
									:disabled="true"
									size="is-medium"
									placeholder="Valor calculado"
									:is-money="true"></input-text>
							</div>
							<div class="column is-half-desktop">
								<select-ajax v-model="form.situacao"
									name="situacao"
						...
						<div class="columns">
							<div class="column is-half-desktop">
								<date-time-picker placeholder="Data da emissão"
							...
							<div class="column is-half-desktop">
								<date-time-picker placeholder="Data do pagamento"
									v-if="form.situacao == 'A'"
									name="dataPagamento"
					...
					<div class="field has-addons has-addons-right is-grouped">
						...
						<div class="control" v-if="form.situacao == 'A'">
							<button type="button" class="button is-black is-medium"
								:class="{ 'is-loading': form.isLoading }"
								@click="onSubmitClose"
								@dblclick.prevent>
								Salvar e Fechar
...
export default {
	components: { DateTimePicker, InputAutocomplete, TabelaAgendamentos, TabelaConsultorias },
	data() {
		return {
			form: new Form({
				id: null,
	...
	methods: {
		...
		onDelete() {
			this.$dialog.confirm({
				...
				type: 'is-danger',
				onConfirm: () => {
					this.form.post('/lancamento/cancelar')
					.then(response => {
						if (response.message) {
							this.$toast.open({
								message: response.message ? response.message : 'Cadastro excluído com sucesso.',
								type: 'is-success',
								position: 'is-top',
								duration: 5000
							});
					...
					.catch(error => {
						if (error.message) {
							this.$toast.open({
								message: error.message ? error.message : 'Erro ao excluir cadastro.',
								type: 'is-warning',
								position: 'is-top',
								duration: 10000
							});
		...
		onSubmit() {
			this.form.post('/lancamento/salvar')
			.then(response => {
				if (response.message) {
					this.$toast.open({
		...
		setFormData(response, error) {
			if (error) {
				...
				for (let field in _.omit(this.form.data(),
					[ 'cliente', 'tecnico', 'agendamentos', 'consultorias' ])) {
	...
	beforeRouteEnter (to, from, next) {
		...
		if (id) {
			Vue.axios.get('/lancamento/findId/' + id)
			.then(response => {
	...
	beforeRouteUpdate (to, from, next) {
		...
		if (id) {
			this.axios.get('/lancamento/findId/' + id)
			.then(response => {
