	...
	<div class="columns is-centered">
		<div class="column">
			...
			<b-message :closable="false">
				<form name="cadastroParametrosForm" method="post" action="#"
					@keydown="form.errors.clear($event.target.name)"
					@change="form.errors.clear($event.target.name)">
					<div class="columns">
						<div class="column is-half-desktop">
							<input-text type="number" name="diasAntecedenciaAgendamentoSemEnvio" 
								v-model="form.diasAntecedenciaAgendamentoSemEnvio" 
								size="is-medium"
								placeholder="Dias de antecedência para agendamento sem envio"
								:has-error="form.errors.has('diasAntecedenciaAgendamentoSemEnvio')"
								:error="form.errors.get('diasAntecedenciaAgendamentoSemEnvio')"></input-text>
						</div>
						<div class="column is-half-desktop">
							<select-ajax name="tipoAntecedenciaAgendamentoSemEnvio" 
								...
								:has-error="form.errors.has('tipoAntecedenciaAgendamentoSemEnvio')"
								:error="form.errors.get('tipoAntecedenciaAgendamentoSemEnvio')"
								v-on:errorSelectAjax="form.setSelectError('tipoAntecedenciaAgendamentoSemEnvio', $event)">
								<option value="" disabled 
									v-html="'Tipo de anteced&ecirc;ncia para agendamento sem envio'"></option>
								<option value="DIAS_CORRIDOS">Dias corridos</option>
								<option value="DIAS_UTEIS" v-html="'Dias &uacute;teis'"></option>
							</select-ajax>
						</div>
					</div>
					<div class="columns">
						<div class="column is-half-desktop">
							<input-text type="number" name="diasAntecedenciaAgendamentoComEnvio" 
								v-model="form.diasAntecedenciaAgendamentoComEnvio" 
								size="is-medium"
								placeholder="Dias de antecedência para agendamento com envio"
								:has-error="form.errors.has('diasAntecedenciaAgendamentoComEnvio')"
								:error="form.errors.get('diasAntecedenciaAgendamentoComEnvio')"></input-text>
						</div>
						<div class="column is-half-desktop">
							<select-ajax name="tipoAntecedenciaAgendamentoComEnvio" 
								v-model="form.tipoAntecedenciaAgendamentoComEnvio" 
								size="is-medium"
								placeholder="Tipo de antecedência para agendamento com envio"
								control-type="is-primary"
								:has-error="form.errors.has('tipoAntecedenciaAgendamentoComEnvio')"
								:error="form.errors.get('tipoAntecedenciaAgendamentoComEnvio')"
								v-on:errorSelectAjax="form.setSelectError('tipoAntecedenciaAgendamentoComEnvio', $event)">
								<option value="" disabled 
									v-html="'Tipo de anteced&ecirc;ncia para agendamento com envio'"></option>
								<option value="DIAS_CORRIDOS">Dias corridos</option>
								<option value="DIAS_UTEIS" v-html="'Dias &uacute;teis'"></option>
							</select-ajax>
						</div>
...
export default {
	...
	methods: {
		onSubmit() {
            this.form.post('/parametros/salvar')
			.then(response => {
				if (response.message) {
					this.$toast.open({
	...
	beforeRouteEnter (to, from, next) {
		Vue.axios.get('/parametros/buscar')
		.then(response => {
			next(vm => {
