	...
	<div class="columns is-centered">
		<div class="column">
			...
			<b-message :closable="false">
				<form name="cadastroTecnicoForm" method="post" action="#" autocomplete="off"
					...
					<div v-show="activeTab == 1">
						<div class="columns">
							<div class="column is-half-desktop">
								...
								<input-autocomplete
									v-if="showAutocompleteEmpresa && !(form.id > 0 && form.autonomo)"
								...
								<b-field v-if="isUsuarioMatrizFilial">
									<b-checkbox v-model="form.autonomo"
								...
								<input-text name="cpf" v-model="form.cpf" 
									type="tel"
									placeholder="CPF" size="is-medium"
									:mask="['###.###.###-##']"
								...
								<b-field v-if="!form.autonomo && (isUsuarioMatrizFilial || isUsuarioEmpresa)">
						...
						<div class="columns" v-show="form.atendeFora">
							...
							<div class="column is-half-desktop">
								<!-- Mapa -->
								<here-maps ref="hereMap" v-if="activeTab == 1 && form.atendeFora"
									:value="markerTecnico"
									@clickBuscarPosicao="buscarEnderecoPosicao"
									@markerPositioned="atualizaEnderecoPosicao($event, form.endereco)"></here-maps>
								<div></div>
							</div>
						</div>
					</div>
					...
					<div v-show="activeTab == 2" v-if="form.autonomo">
						<div class="columns">
							<div class="column is-half-desktop">
								...
								<input-text name="empresaNome" v-model="form.empresa.nome" 
									placeholder="Nome ou razão social" 
									size="is-medium"
									:has-error="form.errors.has('empresaNome')"
									:error="form.errors.get('empresaNome')"></input-text>
								<input-text name="empresaCpfcnpj" v-model="form.empresa.cpfcnpj" 
									type="tel"
									size="is-medium"
									placeholder="CPF/CNPJ"
									:mask="['###.###.###-##','##.###.###/####-##']"
									:has-error="form.errors.has('empresaCpfcnpj')"
									:error="form.errors.get('empresaCpfcnpj')"></input-text>
								<select-ajax v-if="form.empresa.id > 0 && showSelectSituacao"
									name="empresaIdSituacao" v-model="form.empresa.idSituacao"
									size="is-medium"
									...
									v-on:errorSelectAjax="form.setSelectError('empresaIdSituacao', $event)">
								</select-ajax>
								<div class="field" v-if="form.empresa.id > 0 && !showSelectSituacao">
									<label class="label" v-html="'Situa&ccedil;&atilde;o'"></label>
							...
							<div class="column is-half-desktop">
								...
								<!-- FormUsuario -->
								<select-ajax name="idBanco" v-model="form.empresa.idBanco"
								...
								<input-text name="titularConta" v-model="form.empresa.titularConta" 
									placeholder="Nome completo do titular" size="is-medium"
									:has-error="form.errors.has('titularConta')"
									:error="form.errors.get('titularConta')"></input-text>
								<input-text name="cpfTitular" v-model="form.empresa.cpfTitular"
									type="tel"
								...
								<input-text name="agencia" v-model="form.empresa.agencia"
									type="tel"
								...
								<input-text name="conta" v-model="form.empresa.conta"
									type="tel"
	...
	methods: {
		...
		onAutonomoChanged(value) {
			if (value) {
				...
				if (this.form.empresa == null || Object.keys(this.form.empresa).length < 4) {
					this.form.empresa = {
						id: null,
						idSituacao: null,
						situacaoDescricao: null,
