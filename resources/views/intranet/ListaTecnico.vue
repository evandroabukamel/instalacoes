<template>
	<div>
		<h1 class="title is-clearfix">Lista de técnicos</h1>
		<div class="is-clearfix">
			<b-field position="is-right" :addons="true" grouped v-if="!$route.meta.situacao">
			...
			<b-table
				...
				<template slot-scope="props">
					<b-table-column label="Empresa" :visible="isUsuarioMatrizFilial">
						{{ props.row.empresa }}
					</b-table-column>
					<b-table-column field="t.nome" label="Nome" sortable>
						{{ props.row.nome }}
					</b-table-column>
					<b-table-column field="t.cpf" label="CPF" sortable>
						{{ props.row.cpf | formatCpfCnpj }}
					</b-table-column>
					<b-table-column field="e.cidade" label="Localização" sortable>
						{{ props.row.localizacao }}
					</b-table-column>
					<b-table-column field="t.idSituacao" label="Situação" sortable>
						{{ props.row.situacao | capitalize }}
					</b-table-column>
					<b-table-column label="Ações" numeric>
						<router-link :to="{ name: 'cadastro-tecnico', params: {id: props.row.id} }"
...
export default {
	...
	methods: {
		...
		getList() {
			...
			this.axios.get(`/tecnico/getList/page=${this.page}&perPage=${this.perPage}&text=${this.text}`
