<template>
	<b-table
		...
		<template slot-scope="props">
			...
			<b-table-column field="dataAgendamento" label="Data/Hora agendamento" sortable>
				{{ props.row.dataAgendamento|date }} às {{ props.row.horaAgendamento|time }}
			</b-table-column>
		...
		<template slot="empty">
			<section class="section" v-show="!loading">
				<div class="content has-text-grey has-text-centered">
					...
					<p v-show="!Boolean(this.idObjeto)">Selecione o cliente ou o técnico.</p>
					<p v-show="Boolean(this.idObjeto > 0)">Nenhuma ordem de serviço concluída para lançar.</p>
				</div>
			</section>
		</template>
	</b-table>
	...
	methods: {
		...
		getList() {
			if (this.idObjeto > 0 && this.situacao == 'A') {
				this.loading = true;
				this.axios.get(`/lancamento/getAgendamentosConcluidosList/page=${this.page}&perPage=${this.perPage}`
