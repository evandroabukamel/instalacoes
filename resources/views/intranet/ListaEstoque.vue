	...
	<div>
		...
		<div class="is-clearfix">
			<b-field position="is-right" :addons="true" grouped group-multiline>
				<b-select name="classe" v-model="classe"
					@input="getList()"
			...
			<b-table
				...
				<template slot-scope="props">
					...
					<b-table-column field="e.imei" label="IMEI" sortable
						:visible="classe == 'equipamento' || classe == 'chip'">
						<span v-if="Boolean(props.row.imei)">
							{{ props.row.imei | formatImei }}
						</span>
					</b-table-column>
					<b-table-column field="e.iccid" label="ICCID" sortable 
						:visible="classe == 'chip'">
						<span v-if="Boolean(props.row.iccid)">
							{{ props.row.iccid | formatIccid }}
						</span>
					</b-table-column>
					<b-table-column field="e.telefone" label="Telefone" sortable 
						:visible="classe == 'chip'">
						{{ props.row.telefone }}
					</b-table-column>
					<b-table-column field="e.operadora" label="Operadora" sortable 
						:visible="classe == 'chip'">
						{{ props.row.operadora }}
					</b-table-column>
					<b-table-column field="e.numero" label="Número" sortable 
						:visible="classe == 'equipamento'">
						{{ props.row.numero}}
					</b-table-column>
					<b-table-column field="mar.descricao" label="Marca equipamento" sortable 
						:visible="classe == 'equipamento'">
						{{ props.row.marcaEquipamento }}
					</b-table-column>
					<b-table-column field="mod.descricao" label="Modelo equipamento" sortable 
						:visible="classe == 'equipamento'">
					...
					<b-table-column label="Ações" numeric>
						<router-link :to="{ name: 'cadastro-estoque', params: {id: props.row.id} }"
...
export default {
	...
	methods: {
		...
		getList() {
			...
			if (this.classe) {
				this.loading = true;
				this.axios.get(`/estoque/getList/classe=${this.classe}&page=${this.page}&perPage=${this.perPage}`
