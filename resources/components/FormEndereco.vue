	...
	<div>
		...
		<div v-if="!readonly">
			<here-autocomplete
				...
				:size="size"></here-autocomplete>
			<input-text type="text" name="logradouro"
				...
				:has-error="Boolean(error.logradouro)"></input-text>
			<b-field>
				<div class="columns">
					<div class="column is-6-desktop is-12-mobile">
						<input-text type="number" name="numero"
							...
							expanded
							:error="error.numero"
							:has-error="Boolean(error.numero)"></input-text>
					</div>
					<div class="column is-6-desktop is-12-mobile">
						<input-text type="text" name="complemento"
			...
			<input-text type="text" name="bairro"
				...
				:size="size"
				:error="error.bairro"
				:has-error="Boolean(error.bairro)"></input-text>
			<input-text type="text" name="cidade"
				...
				:size="size"
				:error="error.cidade"
				:has-error="Boolean(error.cidade)"></input-text>
			<b-field>
				<div class="columns">
					<div class="column is-6-desktop is-12-mobile">
						<select-ajax name="estado"
							...
							<option v-for="estado in estadosBrasil" 
								:value="estado.value"
								:key="estado.value"
					...
					<div class="column is-6-desktop is-12-mobile">
						<input-text type="tel" name="cep"
							...
							expanded
							:error="error.cep"
							:has-error="Boolean(error.cep)"></input-text>
					</div>
				</div>
			</b-field>
			<span class="help is-danger" v-if="Boolean(error.posicao)" v-text="error.posicao"></span>
		</div>
		<!-- CONSULTA -->
		<div v-else>
			<input-text type="textarea" name="enderecoLinha1"
				v-if="endereco.cidade && endereco.estado"
				:disabled.sync="readonly" :size="size" rows="4"
				:value="`${endereco.logradouro}, ${endereco.numero} ${endereco.complemento}\n` +
					`${endereco.bairro}, ${endereco.cidade} - ${endereco.estado}`">
			</input-text>
			<input-text v-else disabled :size="size" value="Endereço não informado"></input-text>
		</div>
	</div>
</template>
	...
	methods: {
		carregaDadosEndereco(result) {
			...
			if (result.NavigationPosition[0]) {
				...
				this.endereco.posicao.longitude = result.NavigationPosition[0].Longitude;
			}
