<template>
	<div class="content">
		...
		<div id="map" ref="map"
			...
			class="is-expanded" 
			style="height:400px; border: 1px solid #363636;"></div>
	</div>
</template>
...
const mapIcons = require.context('../../static/assets/images/map-icons', true, /\.png$/);
...
export default {
	...
	methods: {
		...
		centerMap(position, address, zoom) {
			if (this.map) {
				if (position) {
					...
					.catch(error => {
						this.hasError = true;
						this.error = 'Erro ao buscar endereço, tente melhorar o endereço buscado.';
					});
				}
			}
		},
		...
		searchPositionForAddress(address) {
			let url = `https://geocoder.cit.api.here.com/6.2/geocode.json?`+
			...
			return new Promise((resolve, reject) => {
				...
				ajax.addEventListener('loadend', function (data) {
					...
					if (json.Response.View.length > 0) {
						this.hasError = false;
						this.error = '';
						resolve(json.Response.View[0].Result[0].Location.NavigationPosition[0]);
					}
					else {
						this.hasError = true;
				...
				ajax.addEventListener("error", function(error) {
					this.hasError = true;
					this.error = 'Erro ao buscar localização, tente melhorar o endereço buscado.';
					reject(this.error);
				});
     		});
		},
	...
	mounted() {
		try {
			...
			this.map.addEventListener('drag', function(ev) {
				...
				if (target instanceof mapsjs.map.Marker) {
					target.setPosition(vm.map.screenToGeo(pointer.viewportX, pointer.viewportY));
				}
			}, false);
			...
			this.hasError = true;
			this.error = 'Não foi possível carregar o mapa. Tente recarregar a página.';
		}
	}
}
</script>
