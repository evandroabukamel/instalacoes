...
if (localStorage.getItem('gts-token')) {
	axios.defaults.headers.get['X-AUTH-TOKEN'] = localStorage.getItem('gts-token');
	axios.defaults.headers.post['X-AUTH-TOKEN'] = localStorage.getItem('gts-token');
}
	...
	try {
		...
		if (error.response.status === 401) {
			if (store.getters.isLoggedIn) {
				Toast.open({
					message: error.response.data.message ? error.response.data.message : 'Necessário fazer login.',
					type: 'is-warning',
					position: 'is-top',
					duration: 5000
				});
			...
			Toast.open({
				message: error.response.data.message ? error.response.data.message : 'Ocorreu algum erro imprevisto no servidor.',
				type: 'is-danger',
				position: 'is-top',
				duration: 7500
			});
