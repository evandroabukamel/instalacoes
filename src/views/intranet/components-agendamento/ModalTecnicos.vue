  ...
  <form>
    <div class="modal-card">
            ...
            <item v-for="(item, index1) in items" :key="index1">
              <div class="tile is-ancestor">
                <div class="tile is-parent is-vertical">
                  ...
                  <div
                    v-for="(agenda, index2) in item.agendas"
                    :key="index2"
                  ...
                  >
                    <div class="notification is-success is-narrow">
                      ...
                      >
                        <div
                          v-for="(os, index3) in agendamentosBetween(
                            item.agendamentos,
                            agenda.horarioInicial,
                            agenda.horarioFinal
                          )"
                  ...
                  <div
                    v-show="!item.agendas || item.agendas.length == 0"
      ...
      <footer class="modal-card-foot">
        <button class="button is-medium" type="button" @click="clickCancel">
          Voltar
        </button>
        <button
...
export default {
  ...
  computed: {
    title() {
      return this.valorServico
        ? `Técnicos da ${this.valorServico.empresa.nome}`
        : '';
    },
  ...
  methods: {
    ...
    agendamentosBetween(agendamentos, start, end) {
      ...
      return _.filter(agendamentos, function (o) {
        ...
        return o != null
          ? time.isBetween(beforeTime, afterTime, null, [])
          : false;
      });
    },
    ...
    horaFinalAgendamento(os) {
      return moment(os.horaAgendamento, 'hh:mm:ss')
        .add(moment.duration(os.tempoPrevisto))
        .format('hh:mm:ss');
    },
        ...
        this.axios
          ...
          .catch((error) => {
            ...
            if (error.response && error.response.data) {
              ...
              this.$dialog.confirm({
                message:
                  'Deseja realizar o agendamento apesar da data e horário indisponíveis?',
                cancelText: 'Não',
                confirmText: 'Sim',
                type: 'is-warning',
                onConfirm: () => {
    ...
    fetchData() {
      if (this.valorServico) {
        ...
        this.axios
          .get(
            `/tecnico/getTecnicosAgendaDia/${this.valorServico.empresa.id}/${this.dataAgendamento}`,
            {
          ...
          .catch((error) => {
            ...
            if (error.response && error.response.data) {
              this.$toast.open({
                message: error.response.data.message
                  ? error.response.data.message
                  : 'Erro ao obter lista de técnicos.',
                type: 'is-warning',
                position: 'is-top',
                duration: 5000,
              });
  ...
  mounted() {
    ...
    if (
      moment(this.dataPreferida).isSameOrBefore(
        this.datePickerConfig.disable[0].to,
        'day'
      ) ||
      !this.dataPreferida
    ) {
      this.dataAgendamento = moment(this.datePickerConfig.disable[0].to)
        .add(1, 'days')
        .format('YYYY-MM-DD');
    } else {
