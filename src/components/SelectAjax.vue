<template>
  <b-field
    :label-for="name"
    :label="Boolean(childValue) ? labelText : null"
    :type="error ? 'is-danger' : fieldType"
    :message="error"
  ...
  methods: {
    ...
    loadList() {
      ...
      if (
        ...
        this.axios
          .get(this.apiRoute + (this.params || ''))
          .then((response) => {
            this.$emit('errorSelectAjax', ''); // Limpa a mensagem de erro
            for (let i in response.data) {
              this.options.push({
                value: response.data[i][this.pkField],
                text: response.data[i][this.textField],
              });
            }
          ...
          .catch((error) => {
            ...
            if (error.response) {
              ...
              this.$emit(
                'errorSelectAjax',
                'Não foi possível carregar este campo. Tente recarregar.'
              );
            }
          });
      }
